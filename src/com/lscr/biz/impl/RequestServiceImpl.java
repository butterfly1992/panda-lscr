		...
		if ((mode_flag == 1 || mode_flag == 2) && lscrnoInterfaceswitch(param)) {// 無接口開關並且判斷參數是否為解鎖請求
			...
			if (mcc.get("nlscr_out_" + param.getImei() + param.getImsi()) == null) {// 获取今天有效解锁弹出广告次数
				...
				if (!param.getImsi().equals("123456789012345")) {// 有卡有效用户
					...
					Utils.log.error(param.getAppid()+",oTappid," + outactuser + ",oFappid,0,0,"+ param.getImei()+","+ param.getImsi()+","+param.getIp());
				} else {// 无sim卡用户
...
//					Utils.log.info(new ObjectArrayMessage(param.getAppid(),param.getImei());
					Utils.log.error(param.getAppid()+",oTappid,0,oFappid," + outactuser + ",0,"+ param.getImei()+","+ param.getImsi()+","+param.getIp());
...
//					}
				...
				effect = Integer.parseInt(mcc.get("nlscr_out_" + param.getImei() + param.getImsi()).toString());
			}
			if (effect > 0) {
				if (limit > 1) {
		 ...
		 */
			...
			if (mcc.get("nlscr_in_" + param.getImei() + param.getImsi()) == null) {// 获取今天有效解锁弹出广告次数
				...
				if (!param.getImsi().equals("123456789012345")) {// 有卡有效用户
					...
					Utils.log.error(param.getAppid()+",iTappid," + inactuser + ",iFappid,0,0,"+ param.getImei()+","+ param.getImsi()+","+param.getIp());
				} else {// 无sim卡用户
					...
					Utils.log.error(param.getAppid()+",iTappid,0,iFappid," + inactuser + ",0,"+ param.getImei()+","+ param.getImsi()+","+param.getIp());
...
//					}
				...
				effect = Integer.parseInt(mcc.get("nlscr_in_" + param.getImei() + param.getImsi()).toString());
			}
			if (effect > 0) {
				if (limit > 1) {
	...
	public Object getObject(Param param, int times, int intimes) {
		...
		try {
			...
			if (mcc.get("nlscr_req" + param.getImei() + param.getImsi()) == null) {// 首次弹出
				...
				if (times > 0) {
					mcc.set("nlscr_out_" + param.getImei() + param.getImsi(), times - 1, expiryd);
				}
				if (intimes > 0) {
					mcc.set("nlscr_in_" + param.getImei() + param.getImsi(), intimes - 1, expiryd);
				}
		...
		try {
			...
			if (mcc.get("nlscr_reqm" + param.getImei() + param.getImsi()) == null) {// 首次请求多个广告展示
				...
				if (!(softs != null && softs.size() == 2)) {// 查询出数据不满足条件
					// mcc.set("nlscr_reqm" + param.getImei() + param.getImsi(),
					// 1);// 下次展示实体广告
					mcc.set("nlscr_apkindex_" + param.getImei() + param.getImsi(), 0);
					return Variable.dataJson;
				}
	...
	public Object getBlacklistResult(Param param) {
		...
		param.setOperation(operators(param.getImsi()));
		try {
			Utils.log.info("balcklist:Id :" + param.getAppid() + ";imei:" + param.getImei() + ";imsi:"
					+ param.getImsi() + ";mode:" + param.getMode() + ";");
